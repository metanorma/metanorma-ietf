    ...
    module Transformer
      module ListTransformer
        ...
        def transform_list_item(item)
          ...
          if li_t.is_a?(Array) && !li_t.empty? && li_content.is_a?(Array) && li_content.all? do |c|
            c.to_s.strip.empty?
          end
            li.content = []
          end
              ...
              if inline
                if inline.is_a?(String)
                  ...
                  if coll_name
                    ...
                    dt_order << Lutaml::Xml::Element.new("Element",
                                                         coll_name.to_s)
                  end
                end
              end
        ...
        def extract_dt_text(dt)
          ...
          if ps.is_a?(Array) && !ps.empty?
            return ps.filter_map { |p| ls_text(p) }.join
          end
