    ...
    module Transformer
      module ReferenceTransformer
        ...
        def transform_constituent(constituent)
          ...
          ids.each do |d|
            next unless ["IETF", "DOI"].include?(d.type)
          ...
          if person&.name
            ...
            if surname
              ...
              if forenames.is_a?(Array) && !forenames.empty?
                parts = forenames.map do |f|
                  ls_text(f).to_s.strip
                end.reject(&:empty?)
                if parts.any?
                  author.initials = parts.map do |p|
                    "#{p[0]}."
                  end.join(" ")
        ...
        def extract_bibitem_annotation(bibitem)
          ...
          texts = notes.filter_map { |n| ls_text(n) }.reject(&:empty?)
