      ...
      module RfcV3ToIetf
        ...
        module TableTransformer
          ...
          def transform_table(table_node)
            ...
            # Table name/title
            title_text = extract_node_title(table_node)
            ...
            # Tbody (RFC XML v3 has at most one tbody)
            tbodies = to_array(table_node.tbody)
            if tbodies.any?
              table.tbody = transform_table_body_section(tbodies.first, :tbody)
            end
          ...
          def transform_table_body_section(section_node, type)
