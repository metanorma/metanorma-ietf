      ...
      module RfcV3ToIetf
        ...
        module TermTransformer
          ...
          def transform_terms_section(section_node)
            ...
            terms_section = Metanorma::StandardDocument::Sections::TermsSection.new(
          ...
          def build_term_from_dt_dd(dt_node, dd_node)
            ...
            term = Metanorma::StandardDocument::Terms::Term.new(id: term_id)
            ...
            # The rfcxml model does not parse the parenthesized domain as a
            # separate attribute — it is embedded in the dt text content.
            open_idx = term_text.rindex("(")
            close_idx = term_text.rindex(")")
            if open_idx && close_idx && close_idx > open_idx
              domain_text = term_text[(open_idx + 1)...close_idx].strip
              term.domain = build_localized_string(domain_text)
            end
          ...
          # Strip trailing parenthesized domain from term designation text.
          # The rfcxml model does not separate the domain from the dt text,
          # so "foo (bar)" must be split into designation="foo" and domain="bar".
          def build_term_designation(text)
            text = text.strip
            idx = text.rindex("(")
            if idx && text.end_with?(")")
              text = text[0...idx].strip
            end

            name = Metanorma::StandardDocument::Terms::TermNameElement.new(
              text: [text],
            )
            expression = Metanorma::StandardDocument::Terms::TermExpression.new(
              name: [name],
            )
            Metanorma::StandardDocument::Terms::Designation.new(
              expression: expression,
            )
          end

          def build_term_definition_from_dd(dd_node, term)
            definition = Metanorma::StandardDocument::Terms::TermDefinition.new
