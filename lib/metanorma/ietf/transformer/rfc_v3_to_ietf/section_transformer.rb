      ...
      module RfcV3ToIetf
        ...
        module SectionTransformer
          ...
          def transform_section(section_node)
            ...
            clause = Metanorma::IetfDocument::Sections::IetfClauseSection.new(
              id: resolve_id(section_node),
            )

            # Title
            title_text = extract_node_title(section_node)
            if title_text && !title_text.empty?
              OrderTracker.set_ordered(clause, :title, build_title_element(title_text))
            end

            # Section attributes
            clause.unnumbered = "true" if section_node.numbered&.to_s == "false"
          ...
          def transform_annex(section_node)
            ...
            annex = Metanorma::IetfDocument::Sections::IetfAnnexSection.new(
              id: resolve_id(section_node),
