      ...
      module RfcV3ToIetf
        ...
        module InlineTransformer
          ...
          def transform_inline_by_attributes(source_node, target)
            ...
            to_array(source_node.sub).each do |s|
              el = Metanorma::Document::Components::Inline::SubElement.new(
                content: extract_rfc_mixed_text(s),
              )
            ...
            # Eref (external URL → link, bibref → eref)
            to_array(source_node.eref).each do |e|
              result = transform_eref(e)
              next unless result
              attr_name, el = result
              target.public_send(:"#{attr_name}=", []) unless target.public_send(attr_name).is_a?(Array)
              target.public_send(attr_name) << el
            end
          ...
          def transform_inline_element(attr_name, node)
            ...
            when :sub
              Metanorma::Document::Components::Inline::SubElement.new(
                content: extract_rfc_mixed_text(node),
              )
            when :sup
          ...
          def transform_eref(eref_node)
            ...
            # RFC XML <eref> is an external URL link → MN <link>
            if target.start_with?("http://", "https://", "ftp://", "mailto:")
              link = Metanorma::Document::Components::Inline::LinkElement.new(
                target: target,
              )
