  ...
  module Ietf
    module Transformer
      module CleanupTransformer
        ...
        INLINE_ATTRS = %i[em strong tt sub sup eref xref relref bcp14 iref cref
                          br u].freeze
        ...
        def walk_all_sections(rfc, &)
          walk_sections(rfc.middle&.section, &) if rfc.middle
          walk_sections(rfc.back&.section, &) if rfc.back
        end
        ...
        def unwrap_single_t_in_li(li)
          ...
          if li_content.is_a?(Array) && li_content.any? do |c|
            c.to_s.strip.empty?
          end && t_content && !t_content.strip.empty?
            li.content = [t_content]
          elsif li_content.nil? || (li_content.is_a?(Array) && li_content.all? do |c|
            c.to_s.strip.empty?
          end)
            li.content = [t_content]
          else
        ...
        def walk_all_text_elements(rfc, &)
          if rfc.front&.abstract
            abs = rfc.front.abstract
            abs.t.each(&) if abs.t.is_a?(Array)
          end

          walk_all_sections(rfc) do |section|
            section.t.each(&) if section.t.is_a?(Array)
            if section.blockquote.is_a?(Array)
              section.blockquote.each do |bq|
                bq.t.each(&) if bq.t.is_a?(Array)
              end
            end
            walk_t_in_lists(section, &)
          end
        end

        def walk_t_in_lists(section, &)
          %i[ul ol].each do |list_attr|
            next unless section.send(list_attr).is_a?(Array)

            section.send(list_attr).each { |list| walk_t_in_li(list, &) }
          end
        end

        def walk_t_in_li(list, &)
          return unless list && list.li.is_a?(Array)

          list.li.each do |li|
            li.t.each(&) if li.t.is_a?(Array)
            li.ul.each { |ul| walk_t_in_li(ul, &) } if li.ul.is_a?(Array)
            li.ol.each { |ol| walk_t_in_li(ol, &) } if li.ol.is_a?(Array)
          end
        end
        ...
        def cleanup_asides_in_lists(section)
          %i[ul ol].each do |list_type|
            ...
            section.send(list_type).each do |list|
              cleanup_asides_in_list_items(list, section)
            end
          end
        end

        def cleanup_asides_in_list_items(list, _section)
          nil unless list && list.li.is_a?(Array)
        ...
        def extract_moved_asides(container, child_attr)
          ...
          aside_children = children.grep(Rfcxml::V3::Aside)
        ...
        UNICODE_PARENT_TAGS = Set.new(%w[t blockquote li dd preamble td th
                                         annotation dt]).freeze
        ...
        def extract_queued_images(text_elem, section)
          ...
          indices_to_extract.each do |item|
            ...
            content[item[:index]] =
              content[item[:index]].gsub("[IMAGE #{item[:image][:number]}]", "")
