      ...
      module TermTransformer
        ...
        def transform_term(term_node)
          ...
          if definition_paragraphs.size > 1
            ol = Rfcxml::V3::Ol.new
            definition_paragraphs.each_with_index do |p, _idx|
              li = Rfcxml::V3::Li.new
              t = transform_paragraph(p)
              next unless t
        ...
        def transform_term_source(source)
          ...
          if origin
            ...
            text += if target
                      "<xref target='#{target}' section='' relative=''/>"
                    else
                      ls_text(origin).to_s
                    end
          end
