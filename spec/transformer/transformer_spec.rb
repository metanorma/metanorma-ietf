...
RSpec.describe Metanorma::Ietf::Transformer do
  describe "example document (RFC)" do
    ...
    let(:expected_xml) do
      File.read("spec/fixtures/transformer/output/example.rfc.xml")
    end
  ...
  describe "antioch document (Internet-Draft)" do
    ...
    let(:expected_xml) do
      File.read("spec/fixtures/transformer/output/antioch.rfc.xml")
    end
  ...
  describe "cleanup transformer unit tests" do
    it "wraps unicode characters in u elements" do
      ...
      if content.none?(Rfcxml::V3::U)
        ...
        expect(content.any?(Rfcxml::V3::U)).to be true
      end
    end
  ...
  # ── Cref cleanup tests (cref cleanup) ───────────────────────────
  ...
  # ── Figure cleanup tests (figure cleanup) ─────────────────────────
  ...
  # ── List attributes tests (list attributes) ────────────────────────
  ...
  # ── Inline bookmark test (inline bookmark) ─────────────────────────
